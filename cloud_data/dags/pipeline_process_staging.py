...
def process_game_stagingdata():
    ...
    game_id = set([(game[1],game[2]) for game in returned_values]) # {('45BX05', 'Peter'), ('36B0WC', 'Max')} - Unique Values
    game_names = set([name[2] for name in returned_values]) # {'Max','Peter'} - Unique Values
    game_data = [(game_data[1],int(game_data[3]),float(game_data[4]),int(game_data[6]),int(game_data[7]),int(game_data[8]),int(game_data[9]),int(game_data[10])) for game_data in returned_values]
    ...
    for game_entry in games_table_content:
        ...
        try:
            ...
            logger.info(f"Failed to add entry '{game_entry[0]}' to {games_table} table")
            failed = True
